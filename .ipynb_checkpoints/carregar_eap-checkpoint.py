...
def _extrair_config(wb) -> dict:
    ...
    # ---- Execução -------------------------------------------------------
    # Bloco execução: linhas 13-15 col L, N, O
    exec_inicio2 = _val(ws, 13, 12)  # L13 — início execução
    exec_dias2   = _val(ws, 13, 14)  # N13 — prazo total dias
    exec_fim2    = _val(ws, 13, 15)  # O13 — fim execução
    exec_decor   = _val(ws, 14, 14)  # N14 — dias decorridos
    exec_hoje    = _val(ws, 14, 15)  # O14 — data referência
...
if __name__ == "__main__":
    ...
    arquivo = sys.argv[1] if len(sys.argv) > 1 else "EAP_MEDICAO_4_MEDICAO___OBRAS__1___2_.xlsx"
